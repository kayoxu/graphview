...
class SugiyamaAlgorithm extends Algorithm {
  ...
  SugiyamaAlgorithm(this.configuration) {
    renderer = SugiyamaEdgeRenderer(
        nodeData, edgeData, configuration.bendPointShape,
        configuration.addTriangleToEdge, configuration);
  }
...
//                    iterator.remove();
  ...
  void median(List<List<Node?>> layers, int currentIteration) {
    if (currentIteration % 2 == 0) {
      for (var i = 1; i < layers.length; i++) {
        ...
        nodeData[n1!]!.median - nodeData[n2!]!.median);
      }
    } else {
        ...
        if (positions.isNotEmpty) {
          ...
          if (positions.length == 1) {
            ...
            median = (positions[(positions.length / 2.0).ceil()] +
                positions[(positions.length / 2.0).ceil() - 1]) ~/ 2;
          }
  ...
  void balance(List<Map<Node, double>> x, List<Map<Node?, double>> blockWidth) {
    ...
    switch (configuration.coordinateAssignment) {
      ...
      case CoordinateAssignment.Center :
      case CoordinateAssignment.Average :

        var isCenter = configuration.coordinateAssignment == CoordinateAssignment.Center;
      ...
      case CoordinateAssignment.UpLeft:
        ...
        break;
      case CoordinateAssignment.Left:
        graph.nodes.forEach((n) {
          coordinates[n] = 10 ?? 0.0;
        });
        break;
      case CoordinateAssignment.Center:
  ...
  void resolveOverlaps(Map<Node, double> coordinates) {
    for (var layer in layers) {
      ...
      layerNodes.sort((a, b) =>
          nodeData[a]!.position.compareTo(nodeData[b]!.position));
        ...
        for (var i = 1; i < layerNodes.length; i++) {
          ...
          if (!nodeData[currentNode]!.isDummy) {
            ...
            if (previousNode != null) {
              leftCoordinate = coordinates[previousNode]! + previousNode.width +
  ...
  Map<int, int> markType1Conflicts(bool downward) {
    if (layers.length >= 4) {
      ...
      /*;
             ...
             */
      for (var i = lower; downward ? i <= upper : i >= upper;
      i += downward ? 1 : -1) {
        var k0 = 0;
  ...
  void horizontalCompactation(Map<Node, Node> align, Map<Node, Node> root,
      Map<Node, Node> sink,
      Map<Node, double> shift, Map<Node, double> blockWidth,
      Map<Node, double> x, bool leftToRight,
  ...
  void placeBlock(Node v, Map<Node, Node> sink, Map<Node, double> shift,
      Map<Node, double> x,
      Map<Node, Node> align, Map<Node, double> blockWidth, Map<Node, Node> root,
      bool leftToRight, List<List<Node>> layers, int separation) {
    if (x[v] == double.negativeInfinity) {
      ...
      try {
        do {
          ...
          final hasPredecessor = leftToRight &&
              positionOfNode(currentNode) > 0 ||
              !leftToRight &&
  ...
  bool isLongEdgeDummy(Node? v) {
    ...
    return nodeData[v!]!.isDummy && successors.length == 1 &&
    ...
    for (var i = 1; i < layers.length - 1; i++) {
      ...
      while (iterator.moveNext()) {
        ...
        if (nodeData[current]!.isDummy) {
          ...
          final bendPoints = edgeData[graph.getEdgeBetween(
              predecessor, current)!]!.bendPoints;
    ...
    graph.nodes.forEach((n) {
      if (nodeData[n]!.isReversed) {
        nodeData[n]!.reversed.forEach((target) {
          final bendPoints = this.edgeData[graph.getEdgeBetween(target, n)!]!
              .bendPoints;
